-- Exact global catalog audit after cash-register writes and operational reporting are applied.
do $audit$
declare
  app_api_oid oid;
...
begin
  ...
  then raise exception 'POST_FINANCE_REQUIRED_OBJECT_MISSING'; end if;

  if (select pg_catalog.count(*) from pg_catalog.pg_class c
      join pg_catalog.pg_namespace n on n.oid = c.relnamespace
      where n.nspname = 'app' and c.relkind = 'r') <> 23
    or (select pg_catalog.count(*) from pg_catalog.pg_class c
      join pg_catalog.pg_namespace n on n.oid = c.relnamespace
      where n.nspname = 'app' and c.relkind = 'r'
        and c.relname = any(expected_app_tables) and c.relrowsecurity and c.relforcerowsecurity) <> 23
    or (select pg_catalog.count(*) from pg_catalog.pg_policies where schemaname = 'app') <> 5
    or (select pg_catalog.count(*) from pg_catalog.pg_proc p
      join pg_catalog.pg_namespace n on n.oid = p.pronamespace
      where n.nspname in ('app_rls','app_private') and p.prosecdef) <> 22
  then raise exception 'POST_FINANCE_SURFACE_REJECTED'; end if;

  if not exists(
    select 1 from pg_catalog.pg_class c
    join pg_catalog.pg_namespace n on n.oid = c.relnamespace
    where n.nspname = 'app_private' and c.relname = 'financial_device_sequences'
      and c.relkind = 'r' and c.relrowsecurity and c.relforcerowsecurity
  ) then raise exception 'POST_FINANCE_PRIVATE_SEQUENCE_REJECTED'; end if;

  foreach function_oid in array expected_security_definer_functions loop
    if not exists (
      select 1 from pg_catalog.pg_proc p
      join pg_catalog.pg_roles owner on owner.oid = p.proowner
      where p.oid = function_oid and p.prosecdef
    ...
    ) then raise exception 'POST_FINANCE_FUNCTION_SECURITY_REJECTED'; end if;
  end loop;
  ...
  foreach function_oid in array allowed_app_api_functions loop
    ...
    then raise exception 'POST_FINANCE_APP_API_EXECUTE_MISSING'; end if;
  end loop;
  ...
  if exists (
    ...
    where n.nspname in ('app_private','app_rls')
      and p.oid <> all(allowed_app_api_functions)
  ...
  then raise exception 'POST_FINANCE_APP_API_SURFACE_REJECTED'; end if;
end
$audit$;
