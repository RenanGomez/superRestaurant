-- Exact global catalog audit after the active KDS ticket read model is applied.
do $audit$
declare
  app_api_oid oid;
...
begin
  ...
  then raise exception 'POST_KDS_REQUIRED_OBJECT_MISSING'; end if;
  ...
  if (select pg_catalog.count(*) from pg_catalog.pg_class c
    ...
    or (select pg_catalog.count(*) from pg_catalog.pg_class c
      ...
      where n.nspname = 'app' and c.relkind = 'r'
        and c.relname = any(expected_app_tables) and c.relrowsecurity and c.relforcerowsecurity) <> 19
    ...
    or (select pg_catalog.count(*) from pg_catalog.pg_proc p
      ...
      where n.nspname in ('app_rls','app_private') and p.prosecdef) <> 16
  then raise exception 'POST_KDS_SURFACE_REJECTED'; end if;
  ...
  foreach function_oid in array expected_security_definer_functions loop
    ...
    ) then raise exception 'POST_KDS_FUNCTION_SECURITY_REJECTED'; end if;
  end loop;
  ...
  foreach function_oid in array allowed_app_api_functions loop
    ...
    then raise exception 'POST_KDS_APP_API_EXECUTE_MISSING'; end if;
  end loop;
  ...
  ) then raise exception 'POST_KDS_APP_API_EXTRA_FUNCTION'; end if;
  ...
  then raise exception 'POST_KDS_APP_API_OBJECT_PRIVILEGE_REJECTED'; end if;
end
$audit$;
