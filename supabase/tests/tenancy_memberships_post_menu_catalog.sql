-- Exact catalog audit after the menu catalog is applied.
-- Role login/password state is verified separately by the lifecycle verifier.
do $audit$
declare
  app_api_oid oid;
  helper_oid oid := pg_catalog.to_regprocedure('app_private.jsonb_has_exact_keys(jsonb,text[])');
...
begin
  ...
  if app_api_oid is null or helper_oid is null
  ...
  then raise exception 'POST_MENU_REQUIRED_OBJECT_MISSING'; end if;

  if (select count(*) from pg_catalog.pg_class c join pg_catalog.pg_namespace n on n.oid=c.relnamespace where n.nspname='app' and c.relkind='r') <> 16
    or (select count(*) from pg_catalog.pg_class c join pg_catalog.pg_namespace n on n.oid=c.relnamespace where n.nspname='app' and c.relname=any(expected_tables) and c.relkind='r' and c.relrowsecurity and c.relforcerowsecurity) <> 16
  then raise exception 'POST_MENU_TABLE_SURFACE_REJECTED'; end if;
  ...
  then raise exception 'POST_MENU_POLICIES_REJECTED'; end if;

  if (select count(*) from pg_catalog.pg_proc p join pg_catalog.pg_namespace n on n.oid=p.pronamespace where n.nspname in ('app_rls','app_private') and p.prosecdef) <> 12
    or exists(
  ...
  then raise exception 'POST_MENU_FUNCTION_SURFACE_REJECTED'; end if;

  if not exists(
    select 1 from pg_catalog.pg_proc p join pg_catalog.pg_roles owner on owner.oid=p.proowner
    where p.oid=helper_oid and not p.prosecdef and p.provolatile='i'
  ...
  ) then raise exception 'POST_MENU_HELPER_REJECTED'; end if;
  ...
  then raise exception 'POST_MENU_SCHEMA_GRANTS_REJECTED'; end if;
  ...
  then raise exception 'POST_MENU_APP_API_OBJECT_PRIVILEGE_REJECTED'; end if;
  ...
  foreach table_name in array expected_tables[1:5] loop
    ...
    then raise exception 'POST_MENU_BASE_TABLE_GRANTS_REJECTED'; end if;
  end loop;

  foreach table_name in array expected_tables[6:16] loop
    if pg_catalog.has_table_privilege('authenticated',pg_catalog.format('app.%I',table_name),'SELECT,INSERT,UPDATE,DELETE')
      or pg_catalog.has_table_privilege('anon',pg_catalog.format('app.%I',table_name),'SELECT,INSERT,UPDATE,DELETE')
      or pg_catalog.has_table_privilege('service_role',pg_catalog.format('app.%I',table_name),'SELECT,INSERT,UPDATE,DELETE')
    then raise exception 'POST_MENU_SERVER_TABLE_GRANTS_REJECTED'; end if;
  end loop;
  ...
  foreach function_oid in array expected_functions loop
    ...
    ) then raise exception 'POST_MENU_PUBLIC_EXECUTE_REJECTED'; end if;
  end loop;
  ...
  foreach function_oid in array expected_functions[1:3] loop
    ...
    then raise exception 'POST_MENU_RLS_FUNCTION_GRANTS_REJECTED'; end if;
  end loop;

  foreach function_oid in array expected_functions[4:12] loop
    if pg_catalog.has_function_privilege('anon',function_oid,'EXECUTE')
      ...
      or pg_catalog.has_function_privilege('service_role',function_oid,'EXECUTE')
    then raise exception 'POST_MENU_PRIVATE_FUNCTION_GRANTS_REJECTED'; end if;
  end loop;
  ...
  foreach function_oid in array allowed_app_api_functions loop
    ...
    then raise exception 'POST_MENU_APP_API_EXECUTE_MISSING'; end if;
  end loop;

  if pg_catalog.has_function_privilege(app_api_oid,expected_functions[10],'EXECUTE')
    or pg_catalog.has_function_privilege(app_api_oid,helper_oid,'EXECUTE')
  ...
  then raise exception 'POST_MENU_HELPER_EXPOSED'; end if;
  ...
  ) then raise exception 'POST_MENU_APP_API_EXTRA_FUNCTION'; end if;
end
$audit$;
