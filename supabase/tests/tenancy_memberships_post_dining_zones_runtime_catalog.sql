-- Exact runtime audit for the schema after dining zones are applied.
-- Run only while app_api has its stable SCRAM LOGIN and no active sessions.
do $audit$
declare
  app_api_oid oid;
...
begin
  ...
  then
    raise exception 'POST_DINING_RUNTIME_REQUIRED_OBJECT_MISSING';
  end if;
  ...
  if exists (
    ...
    or coalesce((
      ...
      from pg_catalog.pg_authid where oid = app_api_oid
    ), true)
    or exists (select 1 from pg_catalog.pg_stat_activity where usename = 'app_api')
  then
    raise exception 'POST_DINING_RUNTIME_APP_API_ATTRIBUTES_REJECTED';
  end if;
  ...
  then
    raise exception 'POST_DINING_RUNTIME_APP_API_CONTAMINATED';
  end if;
  ...
  then
    raise exception 'POST_DINING_RUNTIME_TABLE_SURFACE_REJECTED';
  end if;
  ...
  then
    raise exception 'POST_DINING_RUNTIME_POLICIES_REJECTED';
  end if;
  ...
  then
    raise exception 'POST_DINING_RUNTIME_FUNCTION_SURFACE_REJECTED';
  end if;
  ...
  then
    raise exception 'POST_DINING_RUNTIME_SCHEMA_GRANTS_REJECTED';
  end if;
  ...
  then
    raise exception 'POST_DINING_RUNTIME_APP_API_OBJECT_PRIVILEGE_REJECTED';
  end if;
  ...
  foreach table_name in array array[
    ...
    then
      raise exception 'POST_DINING_RUNTIME_BASE_TABLE_GRANTS_REJECTED';
    end if;
  end loop;
  ...
  foreach table_name in array array['dining_zones', 'dining_zone_audit_events'] loop
    if pg_catalog.has_table_privilege('authenticated', pg_catalog.format('app.%I', table_name), 'SELECT')
      ...
      or pg_catalog.has_table_privilege('service_role', pg_catalog.format('app.%I', table_name), 'DELETE')
    then
      raise exception 'POST_DINING_RUNTIME_DINING_TABLE_GRANTS_REJECTED';
    end if;
  end loop;
  ...
  foreach function_oid in array expected_functions loop
    ...
    then
      raise exception 'POST_DINING_RUNTIME_PUBLIC_EXECUTE_REJECTED';
    end if;
  end loop;

  foreach function_oid in array expected_functions[4:6] loop
    ...
    then
      raise exception 'POST_DINING_RUNTIME_PRIVATE_FUNCTION_GRANTS_REJECTED';
    end if;
  end loop;
  ...
  foreach function_oid in array expected_functions[1:3] loop
    if not pg_catalog.has_function_privilege('authenticated', function_oid, 'EXECUTE')
      ...
      or pg_catalog.has_function_privilege('service_role', function_oid, 'EXECUTE')
    then
      raise exception 'POST_DINING_RUNTIME_RLS_FUNCTION_GRANTS_REJECTED';
    end if;
  end loop;
  ...
  then
    raise exception 'POST_DINING_RUNTIME_APP_API_EXTRA_FUNCTION';
  end if;
end
$audit$;
