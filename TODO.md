...
- [ ] **IN_PROGRESS · P1** — Implementar gateway WebSocket para estados en tiempo real. Nota: ADR-011 fue aprobado y el corte incluye contratos compartidos, persistencia atómica de Order/auditoría/eventos KDS, recuperación REST por cursor y gateway Socket.IO autenticado, filtrado por Restaurant/Branch/estación, con reautorización antes de cada entrega y cero mutaciones por socket. `20260902000200` quedó aplicada persistentemente y su postcheck confirmó 12 versiones exactas, catálogo íntegro, `app_api` runtime, cero sesiones y `db lint` limpio. El runner `verify:orders-realtime:remote` quedó preparado con checkpoints no sensibles para idempotencia, RBAC/tenancy, notificación WebSocket, recuperación durable paginada, aislamiento por estación y revocación; su limpieza normal y `recover:tenancy:remote` validan/eliminan solo el prefijo exacto del `runId`, incluidos Order, auditoría, KDS y cursor. Todo el monorepo pasó lint, typecheck, pruebas y build; CodeGraph quedó actualizado en 195 archivos, 3,627 nodos y 14,409 relaciones. Siguiente paso: autorización separada para una sola E2E remota; si queda verde, mover este slice a REVIEW y comenzar el KDS mínimo. P1 permanece IN_PROGRESS.
