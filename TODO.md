...
- [ ] **IN_PROGRESS · P1** — Implementar mesas, zonas y editor de layout web. Nota: zonas permanece cerrado y no se repitió ninguna migración ni cambio de Data API. `20260901000100` y `20260901000200` están aplicadas; historial remoto de diez versiones, auditor global 5/9/9 y `db lint` sin errores. El runner recuperable cubre creación, permisos, aislamiento, idempotencia, lectura, actualización optimista, versión obsoleta y revocación. Tras dos intentos compensados, el aprovisionador post-mesas corregido estableció la nueva credencial de `app_api`; el postcheck confirmó `runtime`, catálogo íntegro y cero sesiones. La clave secreta moderna quedó presente solo en `.env.adr010.local`, ignorado por Git. La E2E se rechazó antes de crear `runId` porque la clave obtenida por CLI estaba enmascarada; Emmanuel pidió detener esta conversación después de guardar la clave real. Falta ejecutar en una conversación nueva la E2E ya autorizada y luego el smoke visual protegido.
