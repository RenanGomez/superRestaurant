...
- [ ] **IN_PROGRESS · P1** — Implementar mesas, zonas y editor de layout web. Nota: el primer slice deja contrato v1 exacto y `POST /api/v1/dining/zones` implementados con Nest + función PostgreSQL privada, revalidación `tables.manage`, idempotencia, auditoría atómica y respuestas estables 400/403/409/503. `20260831000100` quedó aplicada persistentemente con autorización expresa en `zwbyiefqeujstyzysydn`: dry-run exclusivo, ocho versiones exactas, auditoría global post-zonas verde, `app_api` runtime sin sesiones y `db lint` sin errores. La E2E remota de 119 checks confirmó 201, replay exacto, 409 divergente, 403 por viewer/false pair/revocación con token vivo y cero efectos rechazados; cleanup y postcheck quedaron en cero. Permanece IN_PROGRESS únicamente por mesas, layout y UI.
