...
- [ ] **IN_PROGRESS · P0** — Configurar Auth y acceso definitivo de `apps/api`. Nota: configuración fail-closed, identidad por `auth.getUser`, `APP_GUARD`, lookup PostgreSQL privado con rol `app_api`, scope UUID exacto, selección 200/401/403 y `GET /api/v1/access/memberships` están integrados; las respuestas autenticadas declaran `private, no-store`. El directorio versionado devuelve solo pares Restaurant/Branch activos del actor, nombres y roles, y falla completo con 503 ante PostgreSQL o filas hostiles. El aprovisionador conserva confirmación exacta, contraseña separada, TLS `verify-full`, lock global, SCRAM temporal y compensación. `app_api` continúa `NOLOGIN`, password nula y cero sesiones; Emmanuel autorizó habilitarlo. La contraseña privada de `app_api` ya cumple el contrato fuerte y es distinta de la administrativa. La URL administrativa tiene target/TLS correctos, pero el servidor rechazó su contraseña con SQLSTATE 28P01 en dos preflights; debe actualizarse con la contraseña PostgreSQL vigente antes de cualquier operación remota.
- [ ] **IN_PROGRESS · P0** — Definir esquema/migraciones definitivas de persistencia. Nota: `20260830000100` quedó aplicada persistentemente con autorización expresa en `zwbyiefqeujstyzysydn`; la comparación desde staging confirmó las seis versiones esperadas. Emmanuel autorizó aplicar persistentemente `20260830000200` el 2026-08-30. La migración añade únicamente la función privada/bounded de listado activo de membresías, sin grants de tabla; compiló y pasó auditoría dentro de una transacción real con rollback. Un entrypoint rollback-only dedicado reproduce esa verificación y exige cinco funciones. La aplicación repetirá el staging temporal probado para conservar las cinco migraciones históricas sin reparar `supabase_migrations`, pero dos preflights se detuvieron antes del historial porque PostgreSQL rechazó la contraseña administrativa; no hubo escritura. La autorización sigue vigente y se retomará cuando `ADR010_DATABASE_URL` autentique.
...
- [ ] **IN_PROGRESS · P0** — Configurar login funcional de `apps/web`. Nota: FE-0 contiene Next.js App Router/Tailwind v4, `/login`, `/app`, Auth SSR server-only, refresh/rechazo Nest en `proxy.ts`, cookies endurecidas y configuración fail-closed sin `NEXT_PUBLIC_`. El integrador añadió un harness `node:test` sin dependencias que ejecuta el proxy real con respuestas Next y doble Supabase exclusivo de tests. Lint, typecheck, 23/23 pruebas y build pasaron; las cinco pruebas de proxy demuestran limpieza de cookie, redirect fijo, `no-store`, llamada Nest única y ausencia de ciclo. Solo falta smoke humano real, por lo que continúa IN_PROGRESS. Claude debe detenerse; FE-0.1 sigue BLOCKED hasta despliegue/API runtime y nueva asignación explícita.
...
- [ ] **IN_PROGRESS · P0** — Añadir E2E definitivas de autenticación y selección de restaurante/sucursal. Nota: runner remoto fail-closed implementado para dos usuarios, dos restaurantes y dos sucursales por restaurante; prueba Data API, lookup privado, directorio de membresías y Nest real. El directorio ahora cubre aislamiento por actor, roles revocados, membresía revocada, Branch/Restaurant deshabilitados, token aún válido y `no-store`. Cleanup, recovery, runner y aprovisionador comparten el lock de `app_api`. La ejecución remota completa espera rotar la credencial administrativa, aplicar autorizadamente `20260830000200`, definir `APP_API_PROVISIONING_PASSWORD`, habilitar `LOGIN` y autorizar por separado la exposición de `app`. El 403 por permiso de acción se añadirá al primer endpoint con roles restringidos; `/access/branch` acepta cualquier rol conocido.
