...
- [ ] **REVIEW · P0** — Definir la matriz RBAC versionada. Nota: `RBAC_MATRIX_VERSION=1` publica 18 permisos canónicos P0/P1 y una asignación explícita para los nueve roles, sin herencia implícita. Nest evalúa la unión de roles activos en el scope exacto y falla cerrado ante permisos/roles desconocidos, duplicados o hostiles; `POST /api/v1/access/branch` exige `branch.select` y el primer slice de zonas consume `tables.manage`. La matriz y sus pruebas locales están listas, pendientes de aprobación humana y de evidencia PostgreSQL remota del endpoint restringido.
...
- [ ] **IN_PROGRESS · P1** — Implementar mesas, zonas y editor de layout web. Nota: el primer slice deja contrato v1 exacto y `POST /api/v1/dining/zones` implementados con Nest + función PostgreSQL privada, revalidación `tables.manage`, idempotencia, auditoría atómica y respuestas estables 400/403/409/503. Lint, typecheck, pruebas locales, build y análisis CodeGraph pasan. Permanece IN_PROGRESS: la migración `20260831000100` no se aplicó ni se verificó contra PostgreSQL real, y todavía faltan mesas, layout y UI.
