...
// Local visual verification only: with MOBILE_VISUAL_HARNESS=1 the root screen
// is replaced by the harness in `harness/`. Without the variable Metro never
// resolves that directory, so no build or export can contain it.
const harnessEnabled = process.env.MOBILE_VISUAL_HARNESS === "1";
const harnessRoot = path.resolve(projectRoot, "harness", "harness-root.tsx");
...
config.resolver.resolveRequest = (context, moduleName, platform) => {
  if (harnessEnabled && /(^|[./])src\/ui\/root\.js$/.test(moduleName)) {
    return { filePath: harnessRoot, type: "sourceFile" };
  }
