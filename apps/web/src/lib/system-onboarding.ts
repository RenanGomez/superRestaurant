...
export async function listSystemRestaurants(token: string, apiBaseUrl: string): Promise<readonly SystemRestaurantSummary[] | undefined> {
  try { const response = await fetch(`${apiBaseUrl}/api/v1/system/onboarding/restaurants`, { cache: "no-store", headers: { authorization: `Bearer ${token}` } }); if (!response.ok) return undefined; const value: unknown = await response.json(); if (!Array.isArray(value)) return undefined; return value.filter(isSummary); } catch { return undefined; }
}
