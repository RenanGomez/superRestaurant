...
test("accepts the pinned post-orders, post-KDS, post-finance, and post-P2 catalogs", async () => {
  for (const [auditProfile, auditSql] of [
    ["post_orders_realtime_v1", postOrdersRealtimeAuditSql],
    ["post_kds_v1", postKdsAuditSql],
    ["post_finance_v1", postFinanceAuditSql],
    ["post_p2_v1", postP2AuditSql],
  ] as const) {
    const events: string[] = [];
    await recoverAppApi({
      auditProfile,
...
function recoverySession(
  ...
  options: Readonly<{
    ...
    auditProfile?: "post_orders_realtime_v1" | "post_kds_v1" | "post_finance_v1" | "post_p2_v1";
  }>,
  ...
  return session(async (sql) => {
    ...
    if (sql.includes("roles.oid::text as oid")) {
      assert.equal(
        ...
        options.auditProfile === "post_kds_v1"
          || options.auditProfile === "post_finance_v1"
          || options.auditProfile === "post_p2_v1",
      );
      assert.equal(
        sql.includes("app_private.read_cash_register_operational_report"),
        options.auditProfile === "post_finance_v1" || options.auditProfile === "post_p2_v1",
      );
      assert.equal(
        sql.includes("app_private.list_active_operational_shifts"),
        options.auditProfile === "post_p2_v1",
      );
    ...
    if (
      ...
      || sql.includes("POST_FINANCE_REQUIRED_OBJECT_MISSING")
      || sql.includes("POST_P2_REQUIRED_OBJECT_MISSING")
    ) {
