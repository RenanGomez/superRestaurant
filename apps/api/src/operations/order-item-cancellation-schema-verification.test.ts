...
import {
  verifyOrderItemCancellationSchema,
  type OrderItemCancellationSchemaVerificationDependencies,
} from "./order-item-cancellation-schema-verification.js";
import {
...
const input = Object.freeze({
  ...
  config,
  migrationSql: "begin; create function app_private.persist_order_item_cancellation() returns void; commit;",
...
test("audits the exact base first, verifies the migration rollback-only, then postchecks the base", async () => {
  const calls: string[] = [];
  const dependencies: OrderItemCancellationSchemaVerificationDependencies = {
    ...
    runRollbackVerification: async (options: RunSchemaVerificationOptions): Promise<SchemaVerificationSummary> => {
      ...
      assert.deepEqual(options.expectedSummary, { policies: 5, securedTables: 23, securityDefinerFunctions: 24 });
      return options.expectedSummary as SchemaVerificationSummary;
    },
  };

  const result = await verifyOrderItemCancellationSchema(input, dependencies);
  ...
  assert.deepEqual(result.migrated, { policies: 5, securedTables: 23, securityDefinerFunctions: 24 });
...
test("always performs the read-only postcheck after migration failure", async () => {
  ...
  await assert.rejects(verifyOrderItemCancellationSchema(input, {
...
test("does not attempt the migration when the exact base audit fails", async () => {
  ...
  await assert.rejects(verifyOrderItemCancellationSchema(input, {
    ...
    runRollbackVerification: async () => {
      ...
      return { policies: 5, securedTables: 23, securityDefinerFunctions: 24 };
    },
  }), failure);
